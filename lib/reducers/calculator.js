  ...
  switch (action.type) {
    ...
    case actions.SET_OPERATOR:
      newState = {
        ...
        previousValue: (state.operator && state.operator !== 'C') ? calculate(state.previousValue, parseFloat(state.currentValue), state.operator) : parseFloat(state.currentValue)
      };
    break;
